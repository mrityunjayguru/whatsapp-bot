...
<div class="row">
    ...
    <div class="col-lg-5 col-md-12">
                            ...
                            <select name="employee_id" class="form-select form-select-sm" onchange="this.form.submit()" style="max-width:200px;">
                                ...
                                @foreach($activeEmployees as $employee)
                                    <option value="{{ $employee->id }}" @selected($conversation->assigned_tenant_user_id == $employee->id)>
                                        {{ $employee->is_online ? '🟢' : '⚪' }} {{ $employee->display_name }}
                                    </option>
                                @endforeach
                            </select>
                        </form>
        ...
        <div class="card mb-4 shadow-sm border-0">
            <div class="card-body">
                <div class="section-title">SECTION 4: LOCATION DETAILS</div>
                <div class="row mb-3">
                    <div class="col-6">
...
<div class="card mt-4 mb-4 shadow-sm border-0">
    <div class="card-body">
        ...
        @php
            ...
            $total = $msgStats->count(); 
            $customer = $msgStats->where('direction', 'INBOUND')->count();
            $employee = $msgStats->where('sender_type', 'EMPLOYEE')->count();
            $chatbot = $msgStats->where('sender_type', 'SYSTEM')->count();
        @endphp
...
<div class="row">
    ...
    <div class="col-lg-4 col-md-12 mb-4">
        <div class="card h-100 shadow-sm border-0">
            <div class="card-body">
                <div class="section-title mb-1">SECTION 7: CONVERSATION HISTORY</div>
                ...
                <div class="custom-timeline mt-4">
                    <div class="timeline-item" style="--dot-color: #9ca3af;">
                        ...
                        <div class="timeline-date">{{ $conversation->created_at->format('d M Y, h:i A') }}</div>
                    </div>
                    @if($conversation->assignedUser)
                    <div class="timeline-item" style="--dot-color: #3b82f6;">
                        <div class="timeline-title">Assigned to {{ $conversation->assignedUser->display_name }}</div>
                        <div class="timeline-date">{{ $conversation->created_at->format('d M Y, h:i A') }}</div>
                    </div>
                    @endif
                    @if(in_array($conversation->status, ['RESOLVED', 'CLOSED']))
                    <div class="timeline-item" style="--dot-color: #10b981;">
                        <div class="timeline-title">{{ ucfirst(strtolower($conversation->status)) }}</div>
                        <div class="timeline-date">{{ $conversation->updated_at->format('d M Y, h:i A') }}</div>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="card h-100 shadow-sm border-0">
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    @if($conversation->assignedUser)
                    <div class="d-flex align-items-center">
                        <span class="badge bg-light text-dark rounded-circle me-1 border p-1" style="width:20px;height:20px;display:flex;align-items:center;justify-content:center;">{{ substr($conversation->assignedUser->name, 0, 1) }}</span> <span class="fw-bold small me-1">{{ $conversation->assignedUser->name }}</span>
                    </div>
                    @else
                    <strong>-</strong>
                    @endif
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex align-items-center">
                        <span class="fw-bold small me-1">{{ $conversation->status }}</span> 
                    </div>
                </div>
                <div class="d-flex justify-content-between align-items-center">
                    <span class="text-muted small">Last Activity</span>
                    <strong>{{ $conversation->last_message_at ? \Carbon\Carbon::parse($conversation->last_message_at)->diffForHumans() : '-' }}</strong>
                </div>
            </div>
        </div>
    </div> -->
...
document.addEventListener('DOMContentLoaded', function() {
    // Dynamic dropdown for Country/State in Edit Contact Modal
    const countrySelect = document.getElementById('countrySelect');
    const stateSelect = document.getElementById('stateSelect');
    
    if (countrySelect && stateSelect) {
        function fetchStates(countryName, selectedState = null) {
        ...
        // Initialize state on modal open instead of page load to ensure data is fresh
        const editContactModal = document.getElementById('editContactModal');
        if (editContactModal) {
            editContactModal.addEventListener('show.bs.modal', function () {
    ...
    if (typeof window.Echo !== 'undefined') {
        ...
        window.Echo.channel('conversation.{{ $conversation->id }}')
            .listen('NewMessage', (e) => {
                ...
                // Previously only appended INBOUND messages - meaning a
                // customer's new message showed up live, but the BOT's
                // own auto-reply to it never did (you had to refresh to
                // see it). Now also appends BOT/SYSTEM replies. EMPLOYEE
                // replies are still excluded here on purpose - those are
                // already appended synchronously the moment YOU send
                // them (see the fetch().then() above); including them
                // here too would show your own message twice.
                if (e.message && (
                    e.message.direction === 'INBOUND' ||
                    e.message.sender_type === 'BOT' ||
                    e.message.sender_type === 'SYSTEM'
                )) {
