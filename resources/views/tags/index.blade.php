          ...
          <div class="row g-2 mb-4 align-items-center">
            ...
            <div class="col-md-3 ms-auto">
              <div class="input-group input-group-sm">
                ...
                <input type="text" name="search" class="form-control" placeholder="Search Tag" value="{{ request('search') }}" onkeypress="if(event.key === 'Enter') { document.getElementById('filterForm').submit(); return false; }">
              </div>
            </div>
              ...
              @forelse($tags as $tag)
                <tr>
                  ...
                  <td class="text-center">
                    <a href="{{ route('tags.show', $tag->id) }}" class="btn btn-sm btn-light btn-icon" title="View">
      ...
      <form action="{{ route('tags.store') }}" method="POST">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control @error('tag_name') is-invalid @enderror" id="tag_name" name="tag_name" placeholder="e.g. Enterprise Client" value="{{ old('tag_name') }}" required>
            @error('tag_name')
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Brief description of the tag and its intended use...">{{ old('description') }}</textarea>
            @error('description')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('createdDateFlatpickr')) {
    ...
    function updateSelectedCount() {
      ...
      if (selectedCountText) {
        selectedCountText.innerText = count + ' of ' + totalTags + ' row(s) selected.';
      }
    }

    @if($errors->any())
      if (typeof bootstrap !== 'undefined') {
        var myModal = new bootstrap.Modal(document.getElementById('createTagModal'));
        myModal.show();
      } else if (window.$) {
